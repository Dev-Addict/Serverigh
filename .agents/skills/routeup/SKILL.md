...
name: routeup
description: >
  Install, configure, and use routeup for stable local HTTPS routes, public
  development tunnels, webhook and OAuth callbacks, and browser-agent access
  to local services. Use when the user mentions routeup, wants to replace
  localhost ports with named HTTPS URLs, needs a development service reachable
  from another machine, or wants to inspect local or public HTTP traffic.
license: MIT
compatibility: >
  Requires macOS or Linux. The routeup CLI can be installed by this workflow
...
`https://myapp.localhost`. Expose a route publicly only when an off-machine
browser, webhook provider, OAuth provider, or collaborator must reach it.

## Operating rules

1. Run routeup commands from the project directory. Config discovery only
   checks the current directory.
2. Check for `routeup.json` or a `routeup` block in `package.json` before
   proposing configuration. `routeup.json` wins when both exist.
3. Prefer a local `.localhost` route. Do not expose a service publicly unless
   the user requests and approves public access.
4. For a direct install or setup request, ask approval before running it.
   Otherwise, explain that setup changes the machine trust store and privileged
   port handling, then ask before running it.
5. Never print, commit, or place a routeup token in project configuration. Use
   `ROUTEUP_TOKEN` or private client config written by `routeup setup`.
6. Capture is opt-in and may retain headers and bodies in agent memory. Enable
   it only when requested, and redact authorization, cookie, and signature
   headers.
7. Prefer `--json` for agent-consumed status and readiness output. Do not parse
   styled human output when structured output is available.
...
bypass the prompt. If the execution environment cannot handle an interactive
privilege prompt, ask the user to run the command in a terminal.

For explicitly unprivileged setup, use:

```bash
routeup setup --no-bind --port 8443
```

In that mode, expect local URLs to include the port. Verify setup before
...
Resolve failures reported by `doctor`; do not hide them with insecure TLS
flags.
...
uses `ROUTEUP_NAME`, config `name`, then the current directory basename. See
[configuration patterns](references/configuration.md) before creating or
changing project configuration.

## Choose correct workflow

### Existing process on known port

The local agent starts automatically on the first `routeup serve` or
`routeup expose` call; there is no separate daemon command. Keep the
application in its existing terminal and run:
...
`ROUTEUP_URL`, starts the configured command, waits for readiness, and owns the
process group. The child must bind the supplied `HOST` and `PORT`.

Use `routeup exec` for a supporting worker that needs project URLs and CA
environment but must not claim the route. With no arguments it runs the
configured `script` or `command`; an explicit command after `--` overrides it:
...
server to use; never invent a server URL or token. Configure credentials
without putting secrets into the project:
...
`logs --json` emits NDJSON. Other supported `--json` commands generally emit
one JSON document or readiness event.
...
Captured data is bounded in memory, but it may still contain sensitive
application data. Do not repeat it in summaries unless needed for the task.

## Finish cleanly

Foreground routeup commands own the route, tunnel, or child lifecycle and
should be interrupted normally when the task ends. Stop detached serve with
`routeup stop <name>`.

Do not run `routeup uninstall` as cleanup; uninstalling removes the trusted CA,
privileged-port setup, and all routeup state.
